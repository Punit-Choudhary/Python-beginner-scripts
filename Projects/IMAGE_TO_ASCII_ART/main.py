...
#use this set of tuple when producing high resolution(pixels) ascii images. eg: width=400px
# ASCII_CHARS=("@", "#", "$", "&", "%", "<", ">", "[", "]", "/", ";", ":", "*", ",", ".", " ")

#use this set of tuple when producing low resolution(pixels) ascii image (recommended)
ASCII_CHARS=("@", "#", "$", "&", "%", "/", ";", ":", ",", ".", " ")
...
#-------------------------------------------------------------------------------
#converting the new_image to grayscale using convert --> check the PIL documentation and how to use it
def grayify(image):
    grayscale_image=image.convert('L')
    return grayscale_image

#-------------------------------------------------------------------------------
#converting the image grayscale value to their corresponding ASCII symbol
def pixels_to_ascii(image):
    pixel_value=image.getdata()
    #-----------------------
    #use the code below if youre using ascii tuple intended for high pixel images else comment it
    #characters = "".join([ASCII_CHARS[pixel//17] for pixel in pixel_value])
    #-----------------------
    #use the code below if you're using ascii tuple intended for low pixel images (recommended) else comment it
